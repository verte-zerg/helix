...
const MAX_REGISTER_HISTORY_LEN: usize = 10;
...
impl Register {
    ...
    ///
    /// This iterator's item is a tuple of `(index, values)` where the index can
    /// be passed to [Self::select_history_entry] to select that item. `values` is
    /// a [DoubleEndedExactSizeIterator] of the yank's values.
    ...
    fn select_history_entry(&mut self, index: usize) {
        let Some(length) = self.lengths.remove(index) else { return };
...
impl Registers {
    ...
    pub fn select_history_entry(&mut self, name: char, index: usize) -> Result<()> {
        match name {
            ...
            _ => {
                let Some(register) = self.inner.get_mut(&name) else { return Ok(()) };
...
fn read_from_clipboard<'a>(
    ...
    match provider.get_contents(clipboard_type) {
        Ok(contents) => {
            ...
            let Some(register) = register else { return RegisterValues::new(iter::once(contents.into())) };
...
mod test {
    ...
    fn select_from_register_history_test() {
        ...
        // Three yanks with 'a' as the oldest, 'def' as the newest.
